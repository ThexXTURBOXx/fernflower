...
public class ExprProcessor implements CodeConstants {
  ...
  private static final Map<Integer, Integer> functionMap = Compat.mapOf(
  ...
  public void processBlock(BasicBlockStatement stat, PrimitiveExpressionList data, StructClass cl) {
    ...
    for (int i = 0; i < seq.length(); i++) {
      ...
      Set<Integer> offsets = offset >= 0 ? Compat.setOf(offset) : null;
  ...
  public static void writeArray(StringBuilder sb, int arrayDim, List<TypePathWriteProgress> typePathWriteStack) {
    for (int i = 0; i < arrayDim; i++) {
      Compat.VarReference<Boolean> ref = new Compat.VarReference<Boolean>(true);
      final int it = i;
